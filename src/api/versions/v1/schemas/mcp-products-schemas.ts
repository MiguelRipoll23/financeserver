...
export const FixProductToolSchema = z
  .object({
    id: z
      .number()
      .int()
    ...
    currency_code: z
      ...
      .optional()
      .describe(
        "The ISO 4217 currency code (e.g., EUR, USD). Required when unit_price is provided."
      ),
    price_date: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/)
  ...
  .refine((data) => data.name !== undefined || data.unit_price !== undefined, {
    message:
      "At least one field (name or unit_price) must be provided for update",
  })
  .refine(
    (data) => {
      if (data.unit_price !== undefined) {
        return data.currency_code !== undefined;
      }
      return true;
    },
    {
      message: "currency_code is required when unit_price is provided",
      path: ["currency_code"],
    }
  );
